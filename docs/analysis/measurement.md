...
- [几何属性](../core/geometry-properties.md)：`Area` / `Length` / `Envelope` 等属性的总览
- [空间索引 STRtree](../advanced/spatial-index.md)：用 Envelope 做批量距离查询加速
- [PreparedGeometry](../advanced/prepared-geometry.md)：重复谓词判断的性能优化
- [空间谓词](../predicates/relationships.md)：`Intersects` / `Within` 等关系判断
