...
理解 NTS 的对象模型，是写出正确空间代码的前提。本节自底向上拆解：从 `Coordinate` 一路讲到 `Geometry` 的完整继承树。
...
var seq2 = new PackedCoordinateSequenceFactory()
    .Create(new[] { 0.0, 0.0, 1.0, 1.0 }, 2);  // 一维数组交错存储
```
...
详见 [空间谓词](../predicates/relationships.md)。
...
无效几何在参与运算时可能产生错误结果。修复办法见 [常见问题 FAQ](../cookbook/faq.md) 中的"几何修复"一节。

## 小结

- `Coordinate` 是可变原子，注意共享引用问题
- `CoordinateSequence` 是内部高效存储
- 继承树遵循 OGC SFS：`Point`/`LineString`/`Polygon` + 三个 `Multi*` 集合
- `Polygon` = 1 个 `LinearRing` 外壳 + N 个 `LinearRing` 孔洞
- 方向规则与有效性是 GIS 数据的"卫生"基础
