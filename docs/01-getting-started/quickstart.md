...
注意 `(10,5)`：它在多边形 **边界** 上——`CoveredBy` 返回 `True`，但 `Within` 返回 `False`。这是 OGC 谓词的细节，我们会在 [空间谓词](../03-spatial-relations/relationships.md) 一节详细解释。
...
- [坐标与几何层级](../02-geometry-fundamentals/geometry-hierarchy.md)：理解对象模型
- [API 速查表](../appendix/cheatsheet.md)：常用方法一览
