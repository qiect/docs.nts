...
`PreparedGeometry` 预先构建空间索引和拓扑结构，对反复查询同一几何的场景提速显著。详见 [PreparedGeometry](../06-performance/prepared-geometry.md)。
...
- [PreparedGeometry](../06-performance/prepared-geometry.md)：谓词性能优化
- [空间索引 STRtree](../06-performance/spatial-index.md)：批量查询加速
- [几何属性](../02-geometry-fundamentals/geometry-properties.md)：几何自身的度量与元数据
