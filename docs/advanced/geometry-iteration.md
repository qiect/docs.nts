...
[几何属性页](../core/geometry-properties.md#getgeometryn) 已介绍过基本用法，这里聚焦几个深入点。
...
- [几何属性](../core/geometry-properties.md)：遍历的搭档——了解 `NumGeometries`、`NumPoints`、`Coordinates` 等属性
- [自定义操作与扩展](./extending.md)：用 `IGeometryFilter`、`GeometryTransformer`、`GeometryEditor` 写自定义处理
- [PreparedGeometry 性能优化](./prepared-geometry.md)：批量谓词判断的另一条性能路径
- [仿射变换](../operations/affine-transform.md)：`AffineTransformation` 的完整用法
- [API 速查表](../cookbook/cheatsheet.md)：全站 API 索引
