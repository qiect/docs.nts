...
::: tip 防御性写法：永远带工厂
批量合并入口统一用 `UnaryUnionOp.Union(geoms, factory)`，让空输入返回空几何而非 `null`。空几何是合法对象，可继续参与运算（通常传播空集），下游无需写 `?? empty` 兜底。这与 [几何属性](../core/geometry-properties.md#isempty) 中"空几何是合法几何"一致。
:::
...
混合维度输入的合并结果**类型不固定**：可能是 `GeometryCollection`、`MultiPolygon`、`Polygon`，甚至单一 `Point`（其余都被消解时）。下游代码用 `NumGeometries` + `GetGeometryN` 统一遍历，不要假设固定类型。参考 [几何属性](../core/geometry-properties.md#numgeometries)。
:::
...
::: warning 经纬度下合并需先投影
经纬度（WGS84）坐标下做合并不会报错，但精度模型按"度"运作，细小坐标差会被放大成拓扑问题（细缝、自相交）。建议先投影到米制坐标系（如 CGCS2000 / Web 墨卡托）再合并。详见 [精度模型](../core/precision-model.md)。
:::
...
- [叠加分析 (Overlay)](../operations/overlay.md)：交/并/差/对称差的完整算子与稳健性
- [缓冲区 Buffer](../operations/buffer.md)：圆形配送范围的来源，与合并配合做可达分析
- [凸包与简化](../operations/convex-simplify.md)：合并后降顶点数，降内存与渲染开销
- [空间索引 STRtree](./spatial-index.md)：`UnaryUnionOp` 内部分组加速的底层结构
- [精度模型](../core/precision-model.md)：浮点精度如何影响合并，固定精度合并的原理
